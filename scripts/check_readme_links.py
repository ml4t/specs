...
IMAGE = re.compile(r"!\[[^]]*\]\((?P<target>[^)]+)\)")
USER_AGENT = "ml4t-specs-readme-check/1.0"
...
def targets(readme: Path) -> tuple[str, ...]:
    ...
    matches = [
        (match.start("target"), match["target"].strip())
        for pattern in (LINK, IMAGE)
        for match in pattern.finditer(content)
    ]
    return tuple(dict.fromkeys(target for _, target in sorted(matches)))
